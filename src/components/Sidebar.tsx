...
export const Sidebar: React.FC = () => {
  ...
  const positionClass = direction === 'rtl' ? 'right-0 border-l border-slate-800' : 'left-0 border-r border-slate-800';
  ...
  return (
    <>
      ...
      <aside className={`w-80 bg-slate-900 text-slate-100 flex flex-col h-screen fixed top-0 ${positionClass} z-50 font-sans shadow-2xl transition-transform duration-300 lg:translate-x-0 ${transformClass}`}>
        {/* Brand Header */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-sm text-slate-100 leading-tight">{getSidebarTitle()}</h1>
              <p className="text-xs text-sky-400 mt-0.5">{getSidebarSubtitle()}</p>
            </div>
          </div>
        ...
        <div className="p-4 mx-4 mt-4 bg-slate-800/60 rounded-xl border border-slate-700/50">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-2">
            <ShieldCheck size={14} className="text-sky-400 animate-pulse" />
          ...
          <select
            ...
            className="w-full bg-slate-900 border border-slate-700 text-slate-200 text-xs rounded-lg p-2.5 outline-none focus:border-sky-500 transition-colors cursor-pointer"
          >
          ...
          <div className="mt-2 text-[10px] text-slate-400 p-1.5 rounded bg-slate-900/50">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.id === 'assistant' && (
                  <span className="bg-gradient-to-r from-teal-400 to-sky-500 text-[10px] text-slate-950 px-2 py-0.5 rounded-full font-bold animate-bounce whitespace-nowrap">
        ...
        <div className="p-4 border-t border-slate-800 text-slate-500 text-xs text-center">
          <p className="font-medium text-slate-400 hover:text-slate-300 transition-colors">
